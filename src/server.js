...
// lis le fichier json et renvoit une chaine de caractère qu'on la converti en objet javascript avec json.parse
...
app.use(express.json()); // on envoie du json
...
app.delete("/api/products/:id", (req, res) => {
  const productId = parseInt(req.params.id); // ce que le client demande
  console.log(productId);
  const product = products.find(produit => produit.id === productId)

  if (!product){ // vérifie que c'est pas undefined
    return res.status(404).send("product not found with the given id!")
  }
  const index = products.indexOf(product);
  products.splice(index, 1);
  // on filtre et on enlève ceux qui ne corresponde pas
...
app.get('/api/products/:id', (req, res) => { // : = variable, on récupère la variable en utilisant req.params
  const productId = parseInt(req.params.id); // transforme tout cela en number
  const product = products.find(produit => produit.id === productId);
  if(!product){
